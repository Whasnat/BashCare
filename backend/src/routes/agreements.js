...
export default async function agreementsRoutes(fastify) {
  ...
  fastify.get('/', auth, async (req) => {
    ...
    let query = `SELECT l.*,
                 ...
                 JOIN occupant_profiles tp ON tp.id = l.occupant_id
                 WHERE 1=1`;
    const params = [];
  ...
  fastify.post('/', auth, async (req, reply) => {
    const { unit_id, occupant_id, base_rent, security_deposit, utility_tariff, start_date, end_date, notes } = req.body;
    if (!unit_id || !occupant_id || !base_rent || !start_date) {
      return reply.code(400).send({ error: 'unit_id, occupant_id, base_rent, and start_date are required' });
    }
    ...
    const result = await queryWithRLS(
      ...
      `INSERT INTO agreements
         (landlord_id, unit_id, occupant_id, base_rent, security_deposit, utility_tariff, start_date, end_date, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`,
      [req.user.landlord_id, unit_id, occupant_id, base_rent, security_deposit || 0, utility_tariff || 0, start_date, endDateValue, notes || null]
    );
  ...
  fastify.patch('/:id/terminate', auth, async (req, reply) => {
    const result = await queryWithRLS(
      req.user.landlord_id,
      `UPDATE agreements SET is_active = FALSE, terminated_at = NOW()
